...
static safety_config volkswagen_mlb_init(uint16_t param) {
  ...
  static const CanMsg VOLKSWAGEN_MLB_LONG_TX_MSGS[] = {{MSG_HCA_01, 0, 8, .check_relay = true},
  ...
  static RxCheck volkswagen_mlb_rx_checks[] = {
    ...
    {.msg = {{MSG_ACC_04, 2, 8, 25U, .ignore_checksum = true, .max_counter = 15U, .ignore_quality_flag = true}, { 0 }, { 0 }}},
    {.msg = {{MSG_ACC_02, 2, 8, 25U, .ignore_checksum = true, .max_counter = 15U, .ignore_quality_flag = true}, { 0 }, { 0 }}},
  };
...
static bool volkswagen_mlb_tx_hook(const CANPacket_t *msg) {
  ...
  // Safety check for ACC_05 acceleration/deceleration request (primary long control message on MLB)
  // Signal: ACC_05.ACC_Verz_anf (32|11@1+, scale 0.005, offset -7.22)
  if (msg->addr == MSG_ACC_05) {
    int desired_accel = ((((msg->data[5] & 0x7U) << 8) | msg->data[4]) * 5U) - 7220U;
    if (longitudinal_accel_checks(desired_accel, VOLKSWAGEN_MLB_LONG_LIMITS)) {
      tx = false;
    }
  }
