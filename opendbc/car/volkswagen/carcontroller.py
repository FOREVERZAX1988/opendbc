...
class CarController(CarControllerBase, SnGCarController):
    ...
    if self.CP.openpilotLongitudinalControl:
      ...
      if self.frame % self.CCP.ACC_CONTROL_STEP == 0:
        ...
        if self.CP.flags & VolkswagenFlags.MEB:
          ...
          # ===== Macan(MLB) 纵向控制：纯OP vs 融合 块级分流 =====
          # 用户要求：块级 if/else（非逐信号）。融合关(pure_op)=OP自算ACC02/04/05+LS_01待命；
          # 融合开=完整的OP融合ACC方案(下方原逻辑, 勿改)。
          if self.macan_pure_op:
            # ---------- 纯OP纵向（MacanFusionMode=0）----------
            # 原厂雷达停用：LS_01 bus2 只发待命(不激活)，ACC_05 完全自算执行，
            # ACC_02/04 由 OP 自算显示(雷达停用无 st6 矛盾源)。不用 stock_* 读原厂。
            brake_override = CS.out.brakePressed
            # 纯OP：OP 自己决定 long_active（不依赖原厂 stock status 校准）
            long_active = CC.longActive and not brake_override
            gas_override_stock_p = (getattr(CS, 'pedal_value', 0.0) > 5.0) or CS.out.gasPressed
            acc_control = self.CCS.acc_control_value(CS.out.cruiseState.available, CS.out.accFaulted,
                                                     long_active, gas_override_stock_p)
            torque_active = long_active
            accel = float(np.clip(0.0 if gas_override_stock_p else actuators.accel,
                                  self.CCP.ACCEL_MIN, self.CCP.ACCEL_MAX) if torque_active else 0)
            stopping = actuators.longControlState == LongCtrlState.stopping and not brake_override
            if CS.out.gasPressed:
              stopping = False
              if not CS.out.brakePressed:
                accel = max(accel, 0.1)
            ...
            # 坡度补偿：纯OP 直接读 OP 的 slope_pct（无原厂 ESP 交叉校验需求）
            slope_used = self.slope_pct
            # SnG loes 事件化（与融合同结构，纯OP不依赖原厂 stock_loes）
            if CS.out.gasPressed:
              loes_active = False
            else:
              loes_early_release = CS.out.vEgo > 0.5 or (now_nanos - self.sng_loes_start > 300_000_000)
  ...
  self.packer_pt, self.CAN.pt, CS.acc_type, torque_active, accel,
    ...
    if self.frame % self.CCP.ACC_HUD_STEP == 0 and self.CP.openpilotLongitudinalControl:
      if self.CP.flags & VolkswagenFlags.MEB:
        ...
        if self.macan_pure_op:
          ...
          #   开关：开=视觉+原厂雷达idx 参与；关=纯视觉）；状态由 OP 状态派生。
          op_drel = getattr(CS, 'op_lead_dRel', 0.0)
          op_vlead = getattr(CS, 'op_lead_vLead', 0.0)
          # 距离显示源：MacanRadarFusion 开时用雷达 idx 补强，关时纯视觉
          if self.macan_radar_fusion and getattr(CS, 'stock_lead_distance', 0) > 0:
            lead_distance = getattr(CS, 'stock_lead_distance', 0)
            lead_object = 1 if lead_distance < 1021 else 0
          elif op_drel > 0:
            lead_distance = max(1, min(1021, int(self.op_lead_to_index(op_drel, CS.out.vEgo))))
            lead_object = 1
          else:
            lead_distance = 0
            lead_object = 0
          acc_hud_status = self.CCS.acc_hud_status_value(CS.out.cruiseState.available, CS.out.accFaulted,
                                                         long_active, gas_override_stock)
          set_speed = hud_control.setSpeed * CV.MS_TO_KPH
          can_sends.append(self.CCS.create_acc_hud_control_pure_op(
            self.packer_pt, self.CAN.pt, acc_hud_status, set_speed,
            lead_distance, lead_object, zeitluecke=getattr(CS, 'stock_zeitluecke', 4)))
          # ACC_04 纯OP自算（目标车速度=视觉前车速度 km/h，无目标=满量程327.36）
          lead_speed_kph = (op_vlead * CV.MS_TO_KPH) if op_vlead > 0 else 327.36
          acc_control_p = self.CCS.acc_control_value(CS.out.cruiseState.available, CS.out.accFaulted,
                                                     long_active, gas_override_stock)
          can_sends.append(self.CCS.create_acc_04_control_pure_op(
            self.packer_pt, self.CAN.pt, lead_speed_kph, acc_control_p))
        else:
          lead_distance = getattr(CS, 'stock_lead_distance', 0)
    ...
    # 纯OP模式(macan_pure_op)：SnG/gap_sync 是给原厂 ACC 雷达发的 LS_01 按键帧，
    # 雷达已停用，若代发 RESUME/DIST 会重新激活雷达 —— 故整体禁用（纯OP 的启停由
    # OP 自算 ACC_05 完成，不需原厂雷达参与）。
    # SnG/gap_sync：仅融合模式(非纯OP)生效——纯OP雷达停用，代发 RESUME/DIST 会重新激活雷达。
    if self.CP.openpilotLongitudinalControl and not self.macan_pure_op:
    ...
    # **** 物理 LS_01 转发 bus1->bus2（融合模式） / bus2 待命帧（纯OP）****
    # 融合+纯OP 都需在 OP 纵向时维护 bus2 LS_01 在线（relay 断开后 bus0->bus2 不硬件转发）。
    if self.CP.openpilotLongitudinalControl:
      ...
      #
      # 融合模式(MacanFusionMode=1)：把 bus1 物理拨杆帧按"COUNTER 变化即转发"原样代发到
      #   bus2(雷达侧)，让原厂 ACC 雷达收到真实的物理按键（SET/RESUME/±/DIST 全部）——OP 是
      #   bus2 LS_01 唯一来源，消除双源混叠 → st=6（8913 与用户确认的根因）。
      # 纯OP模式(MacanFusionMode=0)：NOT 复制物理拨杆按键到 bus2——避免激活原厂雷达。
      #   按用户要求，bus2(OP代发 PUT bus 为 CAN.ext)只发 LS_01 待命帧
      #   (LS_Hauptschalter=1, 按键清0)，让原厂 ACC 雷达一直保持待命但不被激活。
      #   bus0/bus1 的 LS_01 信号照常（驾驶员仍可拨杆设定 OP 巡航速度）。
      ...
      if (self.CP.flags & VolkswagenFlags.MLB) and self.CP.openpilotLongitudinalControl:
        if self.macan_pure_op:
          # 纯OP：bus2 只发待命帧（Hauptschalter=1），雷达保持待命不激活。
          if CS.gra_stock_values["COUNTER"] != self.gra_acc_counter_last or True:
            # 每帧/计数变化都保活待命帧（保持 LS_01 bus2 在线）
          ...
          # 原样透传物理拨杆按键（SET/RESUME/±/DIST），不额外代发/重发补偿。
